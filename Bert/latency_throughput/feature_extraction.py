...
# List of features (adjust as needed)
ALL_FEATURES = [
...
def extract_pcap_features(pcap_file, attack_type):
    ...
    feature_list = []
    attack_label = 0 if attack_type.lower() == "normal" else 11  
...
if __name__ == "__main__":
    pcap_file = "Backdoor_attack.pcap" 
    attack_type = "Backdoor Attack"
    ...
    df.to_csv("pcap_features.csv", index=False)
    
    total_time = end_time - start_time
    num_packets = len(df)
    latency_per_packet = total_time / num_packets if num_packets > 0 else float('inf')
    throughput = num_packets / total_time if total_time > 0 else 0
    
    print("Feature extraction complete. Saved to 'pcap_features.csv'.")
    print(f"Total Extraction Time: {total_time:.4f} seconds")
    print(f"Number of Packets Processed: {num_packets}")
    print(f"Average Latency per Packet: {latency_per_packet:.6f} seconds")
    print(f"Throughput (packets/second): {throughput:.2f}")
